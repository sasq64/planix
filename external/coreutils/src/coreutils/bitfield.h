...
class BitField
{
    struct BitRef
    {
        ...
        operator int() const { return bf.get(pos) ? 1 : 0; }
        operator bool() const { return bf.get(pos); }
        BitField& bf;
        int pos;
    };
...
public:
    ...
    BitField(const storage_type& bits)
        : bitsize(bits.size() * 64), bits(bits){};
    ...
    std::vector<uint64_t>& get_vector() { return bits; }

    const void* get() { return &bits[0]; };
    ...
    int first_bit_clr(uint64_t x) const { return __builtin_ctzl(~x); }

    int first_bit_set(uint64_t x) const { return __builtin_ctzl(x); }
